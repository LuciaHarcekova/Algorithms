...
class Solution {
    ...
    public HashMap<Character, Integer> dfs(List<Integer>[] tree, String labels, int[] result, int node, int parent) {
        HashMap<Character, Integer> counter = new HashMap<>();
        for (int child : tree[node]) {
            if (child == parent)
                continue;
            // Call on the children and add their number of occurences to counter
            HashMap<Character, Integer> tmpCounter = dfs(tree, labels, result, child, node);
            tmpCounter.forEach((key, value)
                -> counter.put(key, counter.getOrDefault(key, 0) + value));
        }
        char c = labels.charAt(node);
        counter.put(c, counter.getOrDefault(c, 0) + 1);
        result[node] = counter.get(c);
        return counter;
    }
}