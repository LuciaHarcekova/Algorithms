...
public class Solution {
    public int[] CountSubTrees(int n, int[][] edges, string labels) {
        ...
        Dfs(tree, labels, result, 0, -1);

        return result;
    }

    // Count no. nodes in subtree of nodes which hae the same label
    public Dictionary<char, int> Dfs(List<int>[] tree, String labels, int[] result, int node, int parent) {
        var counter = new Dictionary<char, int>();
        foreach (int child in tree[node]) {
            if (child != parent) {
                // Call on the children and add their number of occurences to counter
                var tmpCounter = Dfs(tree, labels, result, child, node);
                foreach (KeyValuePair<char, int> entry in tmpCounter) {
                    counter[entry.Key] = counter.GetValueOrDefault(entry.Key, 0) + entry.Value;
                }
            }
        }
        char c = labels[node];
        counter[c] = counter.GetValueOrDefault(c, 0) + 1;
        result[node] = counter[c];
        return counter;
    }
}