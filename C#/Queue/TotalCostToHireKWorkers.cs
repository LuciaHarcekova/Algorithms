...
lowest cost is 7 (index 3). The total cost = 4 + 7 = 11. Notice that the worker with index 3 
...
Output: 4
Explanation: We hire 3 workers in total. The total cost is initially 0.
- In the first hiring round we choose the worker from [1,2,4,1]. The lowest cost is 1, and 
we break the tie by the smallest index, which is 0. The total cost = 0 + 1 = 1. Notice that 
workers with index 1 and 2 are common in the first and last 3 workers.
- In the second hiring round we choose the worker from [2,4,1]. The lowest cost is 1 (index 2). 
The total cost = 1 + 1 = 2.
- In the third hiring round there are less than three candidates. We choose the worker from 
the remaining workers [2,4]. The lowest cost is 2 (index 0). The total cost = 2 + 2 = 4.
...
public class Solution {
    public long TotalCost(int[] costs, int k, int candidates) {
        ...
        long totalCost = 0;
        int n = costs.Length;
        int left = n - candidates;
        int right = candidates - 1;
        bool[] visited = new bool[n];

        var queue = new PriorityQueue<(int, int), (int, int)>(Comparer<(int, int)>.Create((a, b) => {
            return a.Item1 != b.Item1 ? a.Item1 - b.Item1 : a.Item2 - b.Item2;
        }));

        for(int i = 0; i < n; i++) {
            if(i <= right || i >= left) {
                queue.Enqueue((costs[i], i), (costs[i], i));
                visited[i] = true;
            }
        }
        ...
        for(int i = 0; i < k; i++) {
            ...
            totalCost += queueItem.Item1;
            var costIndex = queueItem.Item2;

            if(costIndex <= right) {
                right += 1;
                if(right < n && !visited[right]) {
                    queue.Enqueue((costs[right], right), (costs[right], right));
                    visited[right] = true;
                }
            } else if(costIndex >= left) {
                left -= 1;
                if(left >= 0 && !visited[left]) {
                    queue.Enqueue((costs[left], left), (costs[left], left));
                    visited[left] = true;
                }
            }
        }
