...
class Solution(object):
    ...
    def maxProduct(self, root):
        ...
        """
        self.subSums = []
        treeSum = self.sumsTree(root)
        result = max([((treeSum - num) * num) for num in self.subSums])
        return result% self.MOD

    def sumsTree(self, root):
        if (not root): return 0
        treeSum = root.val + self.sumsTree(root.left) + self.sumsTree(root.right)
        self.subSums.append(treeSum)
        return treeSum